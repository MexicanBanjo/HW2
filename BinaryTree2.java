...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        if (node == null) return; // If the node has nothing then return.
        if (node.data == oldVal) node.data = newVal; // If the data within the node is the old value then put in the new value.
    ...
    private int findMinHelper(Node node) {
        if (node == null) return Integer.MAX_VALUE; // Return the max value if there is nothing in the node.
        int leftMin = findMinHelper(node.left); // Left min is assigned as the left node.
        int rightMin = findMinHelper(node.right); // Right min is assigned as the right node.
        return Math.min(node.data, Math.min(leftMin, rightMin)); // The Min is the true min between the left and right minimums.
    ...
    private int nodesGTHelper(Node node, int val) {
        if (node == null) return 0; // If the node has nothing return 0.
        int count = node.data > val ? 1 : 0; // Initialize count to the number of nodes that contain a larger value than val.
    ...
    private int[] averageHelper(Node n) {
        if (n == null) return new int[]{0, 0}; // If the node is null return a new array.
        int[] left = averageHelper(n.left); // Initialize the left as the left node.
        int[] right = averageHelper(n.right); // Initialize the right as the right node.
        int sum = n.data + left[0] + right[0]; // Set the sum to the sum of the data of the right and left.
        int count = 1 + left[1] + right[1]; // Initialize the count of total nodes by adding one.
        return new int[]{sum, count}; // Return an array with the sum and count of nodes.
